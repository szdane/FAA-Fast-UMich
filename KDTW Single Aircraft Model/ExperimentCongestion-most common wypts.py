# -*- coding: utf-8 -*-
"""
Created on Mon May  5 14:23:31 2025
...
def read_in_flight_info(df_path):
	df = pd.read_csv(df_path) #'Yuwei 5-4\\flight_entry_times_with_coords.csv'
...
#%% Run the most-visited nodes experiment
def find_most_visited_nodes(flight_info, conges_df_path):
	times_visited = {}
	for flight in flight_info.index:
		preTRA_entry_coord = ast.literal_eval(flight_info.loc[flight, 'enter_pre_tracon_coord'])
		preTRA_entry_time = pd.to_datetime(flight_info.loc[flight, 'enter_pre_tracon'])
		preTRA_duration = pd.Timedelta(seconds = flight_info.loc[flight, 'duration_seconds'])
		...
		trajectory_nodes = route_to_TRACON(traversal_grid, entry_node, TRACON_node, weight_attribute = 'conges_weight')
		
		# Update times_visited
		for node in trajectory_nodes:
			if node in times_visited.keys():
				times_visited[node] += 1
			else:
				times_visited[node] = 1
				
	popular_nodes = sorted(times_visited.items(), key = lambda x: x[1], reverse = True)
	
	return popular_nodes

#%% — GENERATE VISUALIZATION OF PRE-TRACON AND FLIGHT TRAJECTORY —
# FIX THIS
def observe(traversal_grid, popular_nodes, radius):
	traversal_grid.pos = {(x,y):(x,y) for x,y in traversal_grid.nodes()} # Sets node positions in a grid layout
	
	# Set node sizes for visualization
	nx.set_node_attributes(traversal_grid, 0, "size") #base_node_size, "size")
	for node, num_visits in popular_nodes:
		traversal_grid.nodes[node]['size'] = num_visits*(radius**.5)
	node_sizeList = [traversal_grid.nodes[current_node]['size'] for current_node in traversal_grid.nodes]

	nx.draw_networkx_nodes(traversal_grid, pos = traversal_grid.pos, node_size = node_sizeList, edgecolors = "grey")
	...
	nx.draw_networkx_edges(traversal_grid, pos = traversal_grid.pos, width = .3, style = '-', alpha = .2)
...
flight_info = read_in_flight_info(flight_df_path)

conges_df_path = 'Yuwei 5-12\\grid_counts.csv'
# Initialize times_visited dictionary. key = node, value = times visited
popular_nodes = find_most_visited_nodes(flight_info, conges_df_path)

#%% Export the list of popular nodes
#pd.DataFrame(popular_nodes).to_csv("popular_nodes.csv")

#%% Visualize
# Lift observe function from previous experiment
observe(traversal_grid, popular_nodes, radius)