...
class Base:
    def __init__(
        ...
        if isinstance(origin, str):
            ...
            self.lat1, self.lon1, self.alt1, self.t1 = origin
            self.origin = origin
    ...
    def initial_guess_through_waypoints(self, waypoints: list):
        ...
        `waypoints`: List of (lat, lon, alt_ft) including origin and destination.
        """
        # Total number of segments = len(waypoints) - 1
        n_segments = len(waypoints) - 1
        points_per_segment = self.nodes // n_segments
        leftover = self.nodes % n_segments

        xp_list, yp_list, h_list = [], [], []

        for i in range(n_segments):
            lat1, lon1, alt1, _ = waypoints[i]
            lat2, lon2, alt2, _ = waypoints[i+1]

            n_pts = points_per_segment + (1 if i < leftover else 0) + 1  # add one extra if leftover

            x1, y1 = self.proj(lon1, lat1)
            x2, y2 = self.proj(lon2, lat2)
            h1, h2 = alt1 * ft, alt2 * ft

            xp_seg = np.linspace(x1, x2, n_pts)
            yp_seg = np.linspace(y1, y2, n_pts)
            h_seg = np.linspace(h1, h2, n_pts)
            ...
            xp_list.extend(xp_seg)
            yp_list.extend(yp_seg)
            h_list.extend(h_seg)

        # Ensure final point is included
        xp_list = np.array(xp_list)
    ...
    def initial_guess(self, flight: pd.DataFrame = None): # not used any more
        ...
        if flight is None:
            ...
            xp_0, yp_0 = self.proj(self.lon1, self.lat1) # this means xp and yp are in meters
    ...
    def to_trajectory(self, ts_final, x_opt, u_opt):
        ...
        # mass = self.mass_init - fuel.cumsum() # Update mass based on fuel consumption
