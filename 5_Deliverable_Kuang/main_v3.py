...
N_STEPS_BUDGET = 4                          # Active steps every flight gets after its entry (controls number of middle waypoints)
...
# Grid is sized so that the latest-entering flight has exactly N_STEPS_BUDGET active steps.
# All flights are then capped at the same N_STEPS_BUDGET via the step-budget constraint below.
flights['flight_entry_timestep'] = (flights['rel_entry_time'] / DT).astype(int)  # which global step each flight enters on
max_entry_k = int(flights['flight_entry_timestep'].max())  # latest entry step across all flights
t0       = 0
N_steps  = max_entry_k + N_STEPS_BUDGET + 1  # grid is just wide enough: latest flight uses steps max_entry_k+1 … max_entry_k+N_STEPS_BUDGET
tN       = (N_steps - 1) * DT
M_budget = N_STEPS_BUDGET                    # every flight is capped at this many active steps
...
# This ensures no flight uses more active waypoints than the latest-entering flight has available.
for i in range(N_flights):
...
print(f"  Step budget constraint: exit_k[i] <= entry_k_i + {M_budget} for all flights")
