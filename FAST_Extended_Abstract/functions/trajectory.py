...
class Cruise_with_Multi_Waypoints(Base):
    ...
    def trajectory(self, objective="fuel", **kwargs) -> pd.DataFrame:
        ...
        margin_alt = kwargs.get("middle_alt_margin", 500 * ft) # altitude tolerance # kwargs are defined in the brackets in function(...)
        ...
        for i, (lat_wp, lon_wp, alt_wp_ft, t_wp) in enumerate(middle_waypoints):
            ...
            time_margin = 20  # seconds of tolerance, tune if needed
        ...
        final_time = 1200 #kwargs.get("final_time", None)
        ...
        penalty_weight_heading_rate = 20000 #50 # small weight, tune as needed
        for k in range(self.nodes - 1):
            dpsi = U[k + 1][2] - U[k][2]
            J += penalty_weight_heading_rate * (dpsi ** 2)

        # Penalize rapid change in heading (second derivative of heading angle)
        penalty_weight_heading_accel = 5000 #5  # tune as needed
        ...
        # penalize climb / not enough descent
        idx_alt = 2                 # altitude is the 3rd variable
        weight_altitude = 20     # tune as needed
        for k in range(self.nodes):
            J += weight_altitude * X[k][idx_alt]**2
