# Must be set before any other matplotlib import to avoid Qt platform plugin crash
import matplotlib
matplotlib.use("TkAgg")  # Tkinter backend — supports plt.show() without Qt DLLs
...
df_csv = pd.read_csv(os.path.join(os.path.dirname(__file__), "data", "filtered_rows.csv"),
...
aircraft_list = [
    {"acId": "AAL419_KCLTtoKDTW", "acType": "B737", "waypoint_csv": "FAST_Extended_Abstract/data/flight5.csv"},
...
for ac in aircraft_list:
    ...
    acID = ac["acId"]
    acType = ac["acType"]
    _raw_csv = ac["waypoint_csv"]
    # Normalize legacy paths (e.g. "FAST_Extended_Abstract/data/...") to be relative to this script
    _csv_filename = os.path.basename(_raw_csv)
    waypoint_csv = os.path.join(os.path.dirname(__file__), "data", _csv_filename)
