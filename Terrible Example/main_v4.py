...
# Grid is sized so the latest-entering flight has exactly N_STEPS_BUDGET active steps.
# All flights are capped at the same budget via the step-budget constraint.
flights['flight_entry_timestep'] = (flights['rel_entry_time'] / DT).astype(int)
max_entry_k = int(flights['flight_entry_timestep'].max())  # latest entry step across all flights
t0       = 0
N_steps  = max_entry_k + N_STEPS_BUDGET + 1  # grid just wide enough for latest flight to use all N_STEPS_BUDGET steps
tN       = (N_steps - 1) * DT
M_budget = N_STEPS_BUDGET
print(f"Time steps = {N_steps} loaded  (max_entry_k={max_entry_k}, step budget={M_budget} active steps per flight)...")
print()
...
landed = m.addVars(N_flights, N_steps, vtype=GRB.BINARY, name="landed")   # unconstrained landing flag used exclusively in separation constraints (mirrors stress-test design): the solver can freely set landed=1 to bypass separation for any flight/step, keeping the problem feasible.
...
# Force all flights to have is_end=1 at the final step (every flight must have arrived by the end of the horizon).
# This deactivates separation constraints at the final step when all flights are pinned to their STAR fix.
for i in range(N_flights):
    m.addConstr(is_end[i, N_steps-1] == 1, f"force_arrived_final_{i}")

# Equal step budget: every flight must reach its STAR fix within M_budget steps of its own entry.
# This ensures every flight has the same maximum number of active waypoints.
for i in range(N_flights):
    entry_k_i = int(flights.iloc[i]['flight_entry_timestep'])
    m.addConstr(exit_k[i] <= entry_k_i + M_budget, f"step_budget_{i}")
print("is_end logic constraints created...")
print(f"  Step budget: exit_k[i] <= entry_k_i + {M_budget} for all flights")
...
# vi) Define safety separation constraints
# Uses a separate unconstrained `landed` binary (matching the stress-test design).
# Because `landed` is free, the solver can set landed[i,k]=1 to bypass separation
# when two flights are physically co-located (e.g. at entry or at the same STAR fix).
# This keeps the MILP feasible while still incentivising separation when possible.
    ...
    for i in range(N_flights-1):
        for j in range(i+1,N_flights): # Loop over every pair of aircraft (i, j), without repeating pairs
            if k >= flights.iloc[i]['flight_entry_timestep'] and k >= flights.iloc[j]['flight_entry_timestep']: # Only enforce separation if BOTH aircraft have entered the airspace.
                bin_vars = m.addVars(range(6), name=f'bin_{i}_{j}_{k}', vtype=GRB.BINARY) # binary variables for the 6 possible separation scenarios
...
if m.status == GRB.OPTIMAL: # Only extract results if Gurobi found a valid optimal solution.
    ...
    for i in range(N_flights):
        ...
        print(f"  {'Step':>4}  {'Time (s)':>9}  {'Lat':>10}  {'Lon':>11}  {'Alt (ft)':>10}  {'is_end':>6}  {'landed':>6}")
        print(f"  {'-'*4}  {'-'*9}  {'-'*10}  {'-'*11}  {'-'*10}  {'-'*6}  {'-'*6}")
        for k in range(N_steps):
            t_sec      = k * DT
            lat_val    = x[i][k].X
            lon_val    = y[i][k].X
            ...
            land_val   = int(round(landed[i, k].X))
            marker     = " <-- entry" if k == entry_k else (" <-- ARRIVED" if end_val == 1 and (k == 0 or int(round(is_end[i, k-1].X)) == 0) else "")
            print(f"  {k:>4}  {t_sec:>9.1f}  {lat_val:>10.4f}  {lon_val:>11.4f}  {alt_val:>10.1f}  {end_val:>6}  {land_val:>6}{marker}")
        print()
    ...
    for k in range(N_steps):
        for i in range(N_flights - 1):
            for j in range(i + 1, N_flights):
                ...
                if int(round(landed[i, k].X)) == 1 or int(round(landed[j, k].X)) == 1:
                    continue
                dlat = abs(x[i][k].X - x[j][k].X)
                dlon = abs(y[i][k].X - y[j][k].X)
                dalt = abs(z[i][k].X - z[j][k].X)
    ...
    # 3.2b. Save all decision variables and cost breakdown to CSV
    # Pre-compute chosen fix name per flight
    chosen_fix_per_flight = []
    for i in range(N_flights):
        cf = "?"
        for kf in range(k_fixes):
            if b[i][kf].X > 0.5:
                cf = fix_names[kf]
                break
    ...
    for i in range(N_flights):
        flight_id  = flights.iloc[i]['acId']
        entry_k_i  = int(flights.iloc[i]['flight_entry_timestep'])
        exit_k_val = int(round(exit_k[i].X))
        ...
        for k in range(N_steps):
            ...
            # Cost terms
            smooth = CSMOOTH * (ux[i][k-1].X + uy[i][k-1].X + ALPHA_Z * uz[i][k-1].X) if active_step else 0.0
            accel  = CACCEL  * accel_xy[i, k].X                                          if accel_step  else 0.0
            ...
            var_rows.append({
                ...
                # --- State flags ---
                "is_end":       int(round(is_end[i, k].X))    if k >= entry_k_i else 0,
                "delta_end":    int(round(delta_end[i, k].X)) if k >= entry_k_i else 0,
                "landed":       int(round(landed[i, k].X)),
                # --- Position ---
                "lat":          x[i][k].X,
                "lon":          y[i][k].X,
                "alt_ft":       z[i][k].X,
                # --- Displacements (step k vs k-1) ---
                "dx":           diffx[i, k].X  if active_step else 0.0,
                "dy":           diffy[i, k].X  if active_step else 0.0,
                "dz":           diffz[i, k].X  if active_step else 0.0,
                # --- Absolute displacements (L1 terms) ---
                "ux":           ux[i][k-1].X   if active_step else 0.0,
                "uy":           uy[i][k-1].X   if active_step else 0.0,
                "uz":           uz[i][k-1].X   if active_step else 0.0,
                # --- Speed (2-D, degrees/step) ---
                "speed_2d":     speed[i, k].X  if active_step else 0.0,
                # --- Heading-rate (direction change, step k vs k-1) ---
                "ddx":          ddx[i, k].X    if accel_step else 0.0,
                "ddy":          ddy[i, k].X    if accel_step else 0.0,
                "udx":          udx[i, k].X    if accel_step else 0.0,
                "udy":          udy[i, k].X    if accel_step else 0.0,
                "accel_xy":     accel_xy[i, k].X if accel_step else 0.0,
                # --- Cost terms ---
                "cost_smooth":  smooth,
                "cost_accel":   accel,
