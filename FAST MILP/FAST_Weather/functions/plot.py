...
def plot_3d_trajectories(
    ...
    waypoints=None, waypoints_tolerance_radius=3000, waypoints_tolerance_height=600, plot_waypoint_tol_vol=False,
    ...
    if waypoints is not None:
        ...
        for idx, (acId, wps) in enumerate(waypoints.items()):
            ...
            color = "black" #colors[idx % len(colors)]""
            ax.scatter(np.array(x_wp) / 1000, np.array(y_wp) / 1000, alt_list,
                        color=color, marker='x', s=40, label=f'{acId} Waypoints')
            
            # Draw 3D tolerance cylinders if enabled
            if plot_waypoint_tol_vol:
                for xi, yi, zi in zip(x_wp, y_wp, alt_list):
                    n = 36
                    theta = np.linspace(0, 2*np.pi, n)
                    x_circle = (xi/1000) + (waypoints_tolerance_radius/1000)*np.cos(theta)
                    y_circle = (yi/1000) + (waypoints_tolerance_radius/1000)*np.sin(theta)
                    z_bottom, z_top = zi - waypoints_tolerance_height, zi + waypoints_tolerance_height
                    verts = [[
                        [x_circle[j], y_circle[j], z_bottom],
                        [x_circle[(j+1)%n], y_circle[(j+1)%n], z_bottom],
                        [x_circle[(j+1)%n], y_circle[(j+1)%n], z_top],
                        [x_circle[j], y_circle[j], z_top]
                    ] for j in range(n)]
                    poly = Poly3DCollection(verts, alpha=0.25, color='#F5A216', linewidths=0.3, zorder=5.5)
                    if not label_plotted:
                        poly.set_label("Waypoint Constraint Zone")
                        label_plotted = True
    ...
    # # Force equal X and Y tick spacing
...
def plot_fuel_and_NOx_usage(df1=None, df2=None,
    ...
    """
    Plot cumulative fuel usage and cumulative NOx emissions in a single figure
    with two stacked subplots (shared time axis).

    df1, df2 are expected to have columns:
      - recTime, fuel_used, NOx_emitted
    """
    fig, axs = plt.subplots(2, 1, figsize=(7, 5), sharex=True)
    ...
    if df1 is not None and len(df1) > 0:
        final_time_1 = df1.recTime.iloc[-1]
        final_val_1 = df1.fuel_used.iloc[-1]
        axs[0].scatter(final_time_1, final_val_1, color=color1, zorder=3)
        axs[0].annotate(f"{final_val_1:.0f} kg",
                        xy=(final_time_1, final_val_1),
                        xytext=(final_time_1 - pd.Timedelta(seconds=10), final_val_1 + 20),
                        fontsize=9, color=color1)
    if df2 is not None and len(df2) > 0:
        final_time_2 = df2.recTime.iloc[-1]
        final_val_2 = df2.fuel_used.iloc[-1]
        axs[0].scatter(final_time_2, final_val_2, color=color2, zorder=3)
        axs[0].annotate(f"{final_val_2:.0f} kg",
                        xy=(final_time_2, final_val_2),
                        xytext=(final_time_2 - pd.Timedelta(seconds=10), final_val_2 + 20),
                        fontsize=9, color=color2)
    ...
    if df1 is not None and len(df1) > 0:
        final_time_1 = df1.recTime.iloc[-1]
        final_val_1 = df1.NOx_emitted.iloc[-1]
        axs[1].scatter(final_time_1, final_val_1, color=color1, zorder=3)
        axs[1].annotate(f"{final_val_1:.2f} g",
                        xy=(final_time_1, final_val_1),
                        xytext=(final_time_1 - pd.Timedelta(seconds=10), final_val_1 + 200),
                        fontsize=9, color=color1)
    if df2 is not None and len(df2) > 0:
        final_time_2 = df2.recTime.iloc[-1]
        final_val_2 = df2.NOx_emitted.iloc[-1]
        axs[1].scatter(final_time_2, final_val_2, color=color2, zorder=3)
        axs[1].annotate(f"{final_val_2:.2f} g",
                        xy=(final_time_2, final_val_2),
                        xytext=(final_time_2 - pd.Timedelta(seconds=10), final_val_2 + 200),
                        fontsize=9, color=color2)
