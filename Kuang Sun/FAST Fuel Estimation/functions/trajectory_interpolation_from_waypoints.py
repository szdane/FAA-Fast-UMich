...
class Cruise_with_Multi_Waypoints(Base):
    ...
    def init_conditions(self, **kwargs):
        ...
        y_min = min(yp_0, yp_f) - 100000 #10000
        y_max = max(yp_0, yp_f) + 100000 #10000
    ...
    def trajectory(self, objective="fuel", **kwargs) -> pd.DataFrame:
        ...
        middle_waypoints = kwargs.get("middle_waypoints", [])  # list of tuples [(lat1, lon1), (lat2, lon2), ...]
        ...
        total_distance = geodesic((self.lat1, self.lon1), (self.lat2, self.lon2)).meters

        # Step 1: Rebuild the loop for constraint placement
        for lat_wp, lon_wp in middle_waypoints:
            # Compute geodesic distance from origin to waypoint
            dist_to_origin = geodesic((self.lat1, self.lon1), (lat_wp, lon_wp)).meters

            # Ratio along path
            ratio_along_path = dist_to_origin / total_distance
            wp_index = int(ratio_along_path * (self.nodes - 1))  # round to nearest interior node

            # Project waypoint to Cartesian
        ...
        for k in range(self.nodes - 1):
            ...
            lbg.append([-2 * pi / 180]) 
            ubg.append([2 * pi / 180]) # tunned smooth heading rate change
        ...
        g.append(X[0][2]) 
        lbg.append([alt_target_first - margin_first])
        ubg.append([alt_target_first + margin_first])
        ...
        g.append(X[40][2])
        lbg.append([alt_target_final - margin_final])
        ubg.append([alt_target_final + margin_final])
